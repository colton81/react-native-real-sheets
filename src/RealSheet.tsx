...
import type { ActionSheetProps, RealSheetRef } from './types';
...
export default forwardRef<RealSheetRef, ActionSheetProps>(
  function RealSheet(
    {
      animated = true,
    ...
    const getNextPosition = React.useCallback(
      (snapIndex: number) => {
        return (
          ...
          (actionSheetHeight.current * (snapPoints[snapIndex] ?? 0)) / 100
        );
      },
      [snapPoints],
    );
    ...
    const onSheetLayout = React.useCallback(
      (event: LayoutChangeEvent) => {
        ...
        const nextInitialValue =
          actionSheetHeight.current +
          minTranslateValue.current -
          (actionSheetHeight.current * (snapPoints[currentSnapIndex.current] ?? 0)) /
            100;
    ...
    const snapForward = React.useCallback(
      (vy: number) => {
        ...
        if (getCurrentPosition() === 0) {
          nextSnapPoint = snapPoints[(nextSnapIndex = snapPoints.length - 1)] ?? 0;
        } else {
          for (let i = currentSnapIndex.current; i < snapPoints.length; i++) {
            if (getNextPosition(i) < getCurrentPosition()) {
              nextSnapPoint = snapPoints[(nextSnapIndex = i)] ?? 0;
              break;
            }
          }
        }
    ...
    const panHandlers = React.useMemo(() => {
      ...
      function scrollable(value: boolean) {
        ...
        for (const element of draggableNodes.current) {
          const node = element;
    ...
    const getRef = useCallback(
      (): RealSheetRef => ({
        ...
        keyboardHandler: (enabled?: boolean) => {
          keyboard.pauseKeyboardHandler.current = enabled ?? false;
        },
