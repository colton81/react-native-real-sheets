...
import type { RealSheetRef, Sheets } from './types'
let baseZindex = 999;
// Array of all the ids of ActionSheets currently rendered in the app.
const ids: string[] = [];
const refs: {[name: string]: RefObject<RealSheetRef>} = {};
...
class _SheetManager {
  ...
  registerRef = (
    id: string,
    context: string,
    instance: RefObject<RealSheetRef>,
  ) => {
  ...
  get = <SheetId extends keyof Sheets>(
    id: SheetId | (string & {}),
    context?: string,
  ): RefObject<RealSheetRef<SheetId>> => {
    if (!context) {
    ...
    return refs[`${id}:${context}`] as RefObject<RealSheetRef<SheetId>>;
  };
